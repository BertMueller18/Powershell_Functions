...
.PARAMETER WorkingDirectory
 The directory containing LogStats.csv.
...
.EXAMPLE
PS> .\GetTransactionLogStats.ps1 -Gather -TargetServers "server1","server2"

.EXAMPLE
PS> .\GetTransactionLogStats.ps1 -Gather -TargetServers "server1","server2" -WorkingDirectory "C:\GetTransactionLogStats" -ResetStats
...
function Test-PowerShellVersion
{
    ...
    {
        Write-Warning "[$([DateTime]::Now)] Failed to detect PowerShell version. This script requires at least PowerShell version $MinMajorVersion.$MinMinorVersion."
        return $true
    }
    ...
    {
        Write-Error "[$([DateTime]::Now)] This script requires at least PowerShell version $MinMajorVersion.$MinMinorVersion."
        return $false
    }

    Write-Verbose "[$([DateTime]::Now)] Found acceptable PowerShell version: $($PSVersionTable.PSVersion.Major).$($PSVersionTable.PSVersion.Minor)."
...
function Get-LogGenerationsRemote
{
    ...
    {
        $wsManTest = $null
        $wsManTest = Test-WSMan -ComputerName $server -ErrorAction SilentlyContinue

        if ($null -eq $wsManTest)
        {
            Write-Warning "[$([DateTime]::Now)] Failed to establish Remote PowerShell session to computer $server. To enable PowerShell Remoting, use Enable-PSRemoting, or 'winrm quickconfig' on the server to be configured. No databases on this server will be processed."
            continue
        }

        $job = Invoke-Command -ComputerName $server -ScriptBlock ${function:Get-LogGenerationsLocal} -AsJob
        $allJobs += $job
    }
    ...
    {
        Write-Error "[$([DateTime]::Now)] No Target Servers were reachable via Remote PowerShell. No logs will be processed."
    }
}
...
elseif ($Gather -eq $true -and $TargetServers.Count -eq 0)
{
    Write-Error "[$([DateTime]::Now)] One or more TargetServers must be specified when using the Gather switch."
    return
}

if (!(Test-PowerShellVersion))
{
    Write-Error "[$([DateTime]::Now)] Minimum PowerShell version not present or detectable. Exiting script."
    return
}
...
if ($Gather -eq $true)
{
    Get-LogGenerationsRemote -TargetServers $TargetServers -WorkingDirectory $WorkingDirectory
}
