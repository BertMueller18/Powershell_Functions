...
.DESCRIPTION 
 ...
 Requires PowerShell v3.0 or higher.
...
.EXAMPLE
 >Performs a local log analysis in path 'C:\PerfLogs' and inspects the \LogicalDisk(*)\Avg. Disk Sec/Read and \LogicalDisk(*)\Avg. Disk Sec/Write counters for any computer found within a log.
 PS> .\Get-PerfmonSummaryStats.ps1 -Path "C:\PerfLogs" -Counter "\\*\LogicalDisk(*)\Avg. Disk Sec/Read","\\*\LogicalDisk(*)\Avg. Disk Sec/Write" -Verbose
    ...
    elseif ($files.Count -gt 0)
    {
        ...
        foreach ($file in $files)
        {
            ...
            if ($countersGrouped.Count -gt 0)
            {
                ...
                {
                    if ($counterGroup.Group.Count -gt 1)
                    {
                        #Skip the first sample in the group when measuring, as the first sample on average counters is usually 0, and will throw off the minimum calculation
                        $measured = $counterGroup.Group | Select-Object -Skip 1 | Measure-Object -Property CookedValue -Sum -Maximum -Minimum
                    ...
                    {
                        Write-Verbose "$([DateTime]::Now): Found less than 2 samples for Counter. Skipping. Counter: $($counterGroup.Name)"
                    }
                }
            }
            else
...
function Merge-SummaryGroup
{
    ...
    if ($Group.Count -gt 0)
    {
        ...
        if ($Group[0].CounterType -like "AverageTimer*")
        {
