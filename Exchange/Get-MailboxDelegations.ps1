<#
  ...
  $Mailboxes | foreach {.\Get-MailboxDelegations.ps1 -Mailbox $_ -AdditionalFilteredUsers "*BESAdmin"}
...
Process {
  ...
  if (-not ($Mailbox.RecipientTypeDetails -eq 'LegacyMailbox')) {
    ...
    $AllDelegations = @()
    $FullAccess = Get-MailboxPermission -Identity $Mailbox | Where-Object {($_.AccessRights -like '*FullAccess') -and -not ($_.User -match $FilteredUsersString) -and ($_.Deny -eq $false)}
    $SendAs = Get-ADPermission -Identity $($Mailbox.DistinguishedName) | Where-Object {($_.ExtendedRights -like '*Send-As*') -and -not ($_.User -match $FilteredUsersString) -and ($_.Deny -eq $false)}
