...
(
    ...
    [string[]]
    $DesiredFields = @("client-software", "client-software-version","user-email","client-mode"),
...
function GetClientTypes
{
    ...
    if ($connectionList.Count -gt 0 -and $GroupResultsByDesiredFields -eq $true)
    {
        ...
        {
            $combinedKey = $connection."$($DesiredFields[0])"

            for ($i = 1; $i -lt $DesiredFields.Count; $i++)
            {
                $combinedKey += ("," + $connection."$($DesiredFields[$i])")
            }

            if (($groupedConnections.ContainsKey($combinedKey)) -eq $false)
            {
                $groupedConnections.Add($combinedKey, $connection)
            }
        }
...
if ($null -ne $results)
{
    Write-Verbose "$([DateTime]::Now) Found $($results.Count) relevant log entries."
    ...
    if ($GroupResultsByDesiredFields -eq $true)
    {
        ...
        $groupedConnections.Values | Select-Object -Property $DesiredFields | Export-Csv ClientTypeDiscovery.csv -NoTypeInformation
    }
    else
    {        
